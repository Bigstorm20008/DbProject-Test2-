...
HWND ImageTemplate::getHandle() const
{
	return m_imagehWnd;
}
...
{
	m_hBitmap = (HBITMAP)LoadImage(NULL, TEXT("Images\\defaultImage.bmp"), IMAGE_BITMAP, m_imageWidth, m_imageHeight, LR_LOADFROMFILE);
	SendMessage(m_imagehWnd, STM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)m_hBitmap);
}
void ImageTemplate::createButtonForImage(TCHAR* buttonText, UINT buttonIdentifier)
{
	...
	m_editForImageLocation = new EditControlTemplate(m_parentOfImage, editXPos, editYPos, editWidth, editHeight, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_READONLY | ES_MULTILINE | ES_AUTOHSCROLL);
}
...
void ImageTemplate::setNewImageLocationAndLoadIt(TCHAR* imageLocation)
{
	...
	HBITMAP hBitmap = (HBITMAP)LoadImage(NULL, imageLocation, IMAGE_BITMAP, m_imageWidth, m_imageHeight, LR_LOADFROMFILE);
	if (hBitmap)
	{
		m_hBitmap = hBitmap;
		SendMessage(m_imagehWnd, STM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)m_hBitmap);
